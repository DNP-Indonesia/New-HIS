...
class m_pembelian extends CI_Model
{
    ...
    {
        return $this->db->from($this->table3)
            ->join('sdr_request_sundries', 'sdr_request_sundries.faktur = '.$this->table3.'.faktur')
            ->join('sdr_barang', 'sdr_barang.id_barang = '.$this->table3.'.id_barang')
            ->where('sdr_request_sundries.status', 'Disetujui2')
            ->where($this->table3.'.statuskeranjang', 'tidak')
            ->get()
            ->result();
    }
